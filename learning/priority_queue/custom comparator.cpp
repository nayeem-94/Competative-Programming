...
struct cmp
{
    bool operator()(const int &a, const int &b) // here intially a < b
    {
        // If parity is different, even comes first
        if ((a % 2 == 0) && (b % 2 != 0)) return false; // a is even → a < b
        if ((a % 2 != 0) && (b % 2 == 0)) return true;  // b is even → b < a

        // If both are even or both are odd, normal min-heap
